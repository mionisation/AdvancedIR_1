 ...
 */
public class BM25SimilarityVA2 extends Similarity {
   ...
   */
  public BM25SimilarityVA2(float k1, float b) {
  ...
  public final SimWeight computeWeight(CollectionStatistics collectionStats, TermStatistics... termStats) {
    ...
    for (int i = 0; i < cache.length; i++) {
      //ORIGINAL: cache[i] = k1 * ((1 - b) + b * decodeNormValue((byte)i) / avgdl);
      //CHANGE the B calculation with the formula proposed in the paper
      cache[i] = k1 * (1/(mavgtf * mavgtf) * decodeNormValue((byte) i) / Td) +
   ...
   * @see #BM25SimilarityVA2(float, float)
   */
